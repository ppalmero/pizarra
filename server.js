...
io.sockets.on('connection', function(socket) {
	...
	socket.on('goma',function(){
		console.log('Usando goma');
		io.sockets.emit('goma',true);
	});
	
	socket.on('marcador',function(){
		console.log('Usando marcador');
		io.sockets.emit('marcador',true);
	});
